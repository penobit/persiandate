...
class CalendarUtils {
    ...
    /**
     * Converts a PersianDate date to Gregorian.
     *
     * @param int $jy
     * @param int $jm
     * @param int $jd
    ...
    public static function persianCal($jy) {
        ...
        if ($jy < $jp || $jy >= $breaks[$breaksCount - 1]) {
            throw new \InvalidArgumentException('Invalid PersianDate year : ' . $jy);
        }
    ...
    public static function date($format, $stamp = false, $timezone = null) {
        ...
        list($jYear, $jMonth, $jDay) = self::toPersianDate($year, $month, $day);
