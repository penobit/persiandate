...
use DateTime;
use DateTimeZone;
use Penobit\PersianDate\CalendarUtils;
...
class CalendarUtilsTest extends TestCase
{
    ...
    public function testToPersianDate()
    {
        $this->assertTrue(CalendarUtils::toPersianDate(2016, 5, 7) === [1395, 2, 18]);
        $this->assertFalse(CalendarUtils::toPersianDate(2015, 5, 7) === [1394, 2, 18]);
    }
    ...
    public function testIsLeapPersianDateYear()
    {
        $this->assertTrue(CalendarUtils::isLeapPersianDateYear(1395));
        $this->assertFalse(CalendarUtils::isLeapPersianDateYear(1394));
    }
    ...
    {
        $persianDate = '1393/03/27';
        $date = CalendarUtils::parseFromFormat('Y/m/d', $persianDate);
    ...
    public function testCreateCarbonFormFormat()
    {
        ...
        $jalaiDateFormatted = PersianDate::fromDateTime($carbon->toDateString())->format('Y-m-d H:i:s');
        $jalaiDateTimeFormatted = PersianDate::fromDateTime($carbon->toDateTimeString())->format('Y-m-d H:i:s');
